...
Defaults: `period-size` 512, `period-num` 64, `headroom` 512. Smaller `PERIOD_SIZE` → lower latency, more risk of underruns; larger → the opposite. The tray menu goes down to **16** frames (very demanding on USB — use for experiments, then back off if you hear glitches). After `install-usb.sh`, the session tray (`tools/open-apollo-tray.py`) can change the USB period from the menu. Autostart is registered when AppIndicator is present at install time; if you installed `gir1.2-appindicator3-0.1` later, run **`bash scripts/install-open-apollo-tray-autostart.sh`** as your user (no sudo), then log out/in.
