...
- **Session tray** — `install-usb.sh` pulls **GTK 3 + AppIndicator** (tray) and **GTK 4 + Libadwaita** (standalone **USB mixer** window) on **apt** / **dnf** (and **pacman** for the mixer typelibs), then registers autostart for `tools/open-apollo-tray.py` (Solo USB + PipeWire status; **Buffer size (USB)** + **USB mixer…**). If autostart was skipped, install the packages and run **`bash scripts/install-open-apollo-tray-autostart.sh`** as your user (no sudo), then log out/in. Standalone mixer: **`python3 tools/open-apollo-usb-mixer.py`** — Libadwaita UI (48V, mic/line, experimental PAD/low-cut, gain, headphone monitor). Optional menu integration: edit `Path=` in **`data/applications/org.openapollo.UsbMixer.desktop`**, then copy that file and **`data/metainfo/org.openapollo.UsbMixer.metainfo.xml`** into `~/.local/share/applications/` and `~/.local/share/metainfo/` (validate with `appstreamcli validate` if installed).
...
The installer handles dependencies, firmware setup, kernel module build, PipeWire configuration, and (depending on mode) **light vendor init** or **full legacy DSP init**. You'll need the Apollo firmware file from UA's website — the installer will prompt you if it's missing. With GTK 3 / AppIndicator bindings available, it also drops a **tray autostart** (`~/.config/autostart/open-apollo-tray.desktop`) so you can see plug-in status and change the USB ALSA buffer from the menu. The **USB mixer** window uses **GTK 4 + Libadwaita** (`gir1.2-adw-1` / `libadwaita`). Install those packages if the tray’s “USB mixer…” fails to open; you can still run `python3 tools/open-apollo-tray.py` manually once the bindings exist.
